...
class Question0:
     def __init__(self):
         ...
         self.duree_str=convert_sci(self.duree)
         self.puissance_str=convert_sci(self.puissance)
         #
         ...
         #
         if choix==1:
             self.enonce="<p>Un objet se déplace sur une surface plane sous l'effet d'une force de traction de \(%s [N]\). "%(self.force_str)
             self.enonce+="La force de traction forme un angle de \(%s°\) vers le haut par rapport à l'horizontale et "%(self.angle_str)
             self.enonce+="l'objet se déplace de \(%s %s\) en \(%s %s\).</p>"%(self.deplacement_str,self.unites_deplacement,self.duree_str,self.unites_duree)
             self.enonce+="<p>Calcule la puissance avec laquelle cette action est accomplie.</p>"
             
             self.feedback=""
             self.feedback+="<p>\(P=\\frac{W}{\Delta t}\) et \(W=F \cdot \Delta x \cdot cos \ \\alpha\)</p>"
             self.feedback+="<p>Donc : \(P=\\frac{F \cdot \Delta x \cdot cos \ \\alpha}{\Delta t}\)</p>"
             #
             self.feedback+="<p>\(P=\\frac{%s \cdot %s \cdot cos \ (%s)}{%s}\)</p>"%(self.force_str,self.deplacement_si_str,self.angle_str,self.duree_str)
             self.feedback+="<p>\(P=%s [W]\)</p>"%(self.puissance_str)
             self.reponse='\(P=%s [W]\)' %(self.puissance_str)
             self.reponses_fausses=[]
             for facteur in ponderateur:
                 self.reponses_fausses.append("\(P=%s [W]\)"%(convert_sci(self.puissance*facteur)))
             #
         ###
         if choix==2:
             self.enonce="<p>Une machine d'une puissance de \(%s [W]\) est capable de déplacer, sur une surface plane, un objet en \(%s [s]\) sur une distance de \(%s [m].</p>"%(self.puissance_str,self.duree_str,self.deplacement_str)
             self.enonce+="<p>Si la force que cette machine exerce forme un angle de \(%s °\) vers le haut par rapport à l'horizontale, quelle est l'intensité de cette force?</p>"%(self.angle_str)
             
             self.feedback=""
             self.feedback+="<p>\(P=\\frac{W}{\Delta t}\) et \(W=F \cdot \Delta x \cdot cos \ \\alpha\)</p>"
             self.feedback+="<p>Donc : \(P=\\frac{F \cdot \Delta x \cdot cos \ \\alpha}{\Delta t}\)</p>"
             self.feedback+="<p>Et donc : \(F=\\frac{P \cdot \Delta t }{\Delta x \cdot cos \ \\alpha}\)</p>"
             self.feedback+="<p>\(F=\\frac{%s \cdot %s }{%s \cdot cos \ (%s)}\)</p>"%(self.puissance_str,self.duree_str,self.deplacement_si_str,self.angle_str)
             self.feedback+="<p>\(F= %s[N]\)</p>"%self.force_str
             
             self.reponse='\(F= %s [N]\)' %(self.force_str)
             self.reponses_fausses=[]
             for facteur in ponderateur:
                 self.reponses_fausses.append("\(F= %s [N]\)"%(convert_sci( self.force*facteur)))
         ###
         if choix==3:
...
if __name__=="__main__":
     ...
     f = open('./exe_puissance.xml','w')
     
     f.write('<?xml version="1.0" encoding="UTF-8"?> \n')
     f.write('<quiz>  \n')
     f.write('<question type="category">  \n')
     f.write('<category>  \n')
     f.write('<text>$course$/Physique/Puissance</text>  \n')
     f.write('</category>  \n')
     f.write('</question>  \n')
     for question in questionnaire:
         f.write('<question type="multichoice">  \n')
         f.write('<name>  \n')
         f.write('<text>Puissance</text>  \n')
         f.write('</name>  \n')
